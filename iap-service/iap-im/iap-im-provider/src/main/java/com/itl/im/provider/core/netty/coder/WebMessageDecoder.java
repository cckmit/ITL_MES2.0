package com.itl.im.provider.core.netty.coder;

import iap.im.api.sendDto.SentBody;
import com.google.protobuf.InvalidProtocolBufferException;
import iap.im.api.proto.SentBodyProto;
...
public class WebMessageDecoder extends SimpleChannelInboundHandler<Object> {
    ...
    private static final ConcurrentHashMap<String, WebSocketServerHandshaker> HAND_SHAKER_MAP = new ConcurrentHashMap<>();
    ...
    private void handleHandshakeRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
        ...
        HAND_SHAKER_MAP.put(ctx.channel().id().asLongText(), handShaker);
    ...
    private void handlerCloseWebSocketFrame(ChannelHandlerContext ctx, CloseWebSocketFrame frame){
        WebSocketServerHandshaker handShaker = HAND_SHAKER_MAP.get(ctx.channel().id().asLongText());
